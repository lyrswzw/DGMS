...
def get_loader(path, batch_size):
    ...
    dataloader = {x: DataLoader(dataset[x], batch_size=batch_size,  shuffle=shuffle[x], num_workers=0)
                  for x in ['train', 'test']}
    ...
    input_data_par['img_val'] = img_test
    input_data_par['text_val'] = text_test
    input_data_par['label_val'] = label_test
